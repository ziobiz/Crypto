...
async function loadOperatingFeeRates(opts?: {
  feeShareRaw?: unknown;
  ...
  const resolved = await customerFeePolicyService.resolveCustomerFeeShare({
    ...
    feeShareRaw: opts?.feeShareRaw,
    asOf: opts?.asOf,
  });
  const totals = sumOrgShareTable(resolved.share.USDT_PURCHASE);
  return {
    operatingFeePercent: totals.poolPercent,
    operatingFeeFixedUsdt: totals.perTicketUsdt,
  };
}

export async function loadOperatingFeeRatesForQuote(opts?: {
  feeShareRaw?: unknown;
  customerProfileId?: string | null;
  asOf?: Date;
}) {
  return loadOperatingFeeRates(opts);
}
