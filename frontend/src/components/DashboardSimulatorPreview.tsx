  ...
  return (
    <ContentCard title={t('dashboard.simPreviewTitle')}>
      ...
      <div className="space-y-2">
        {rows.map((row) => {
          const showTotalFee = row.feeDiagramDisplay?.showTotalFee !== false;
          const rangePct = row.amountRangePct;
          const showRange = rangePct != null && rangePct > 0;
          const net = Number(row.netUsdt);
          const usdtDelta = showRange ? net * (rangePct / 100) : 0;
          const usdtLow = Math.max(0, net - usdtDelta);
          const usdtHigh = net + usdtDelta;
          return (
